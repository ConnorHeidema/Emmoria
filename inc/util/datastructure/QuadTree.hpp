...
public:
	...
	std::list<std::shared_ptr<QuadNode>> RetrieveAll();
	 ...
	 */
	void emplace_front(std::shared_ptr<QuadNode> pQuadNode);
...
private:
	 ...
	 * @return the average
	 */
	int Average_(int const& first, int const& second);

	std::list<std::shared_ptr<QuadNode>> m_rootNodePtrList;

	std::array<std::unique_ptr<QuadTree>, NUM_CORNERS> m_subTrees;
	...
	unsigned int const k_uBreakingNodeSize;
};
